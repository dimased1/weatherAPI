...
async function callHuggingFace(text) {
  ...
  const body = JSON.stringify({ inputs: `Сгенерируй короткий и понятный прогноз погоды: ${text}` });
  ...
  if (ctype.includes('application/json')) {
    ...
    // possible shapes: { generated_text: '...' } or [{generated_text:'...'}] or [{'generated_text':...}] or array of strings
...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      // fallback: craft a short human message
    ...
    res.status(200).json({ city: extracted.city || city, human_forecast: typeof hfOutput === 'string' ? hfOutput.trim() : JSON.stringify(hfOutput) });
  } catch (err) {
